...
// add your module to this map so it gets used
    ...
    var getStoredLanguage = function () {
        return localStorage && localStorage.getItem(LS_LANG);
    };
    ...
    var getLanguage = messages._getLanguage = function () {
        if (window.cryptpadLanguage) {
            return window.cryptpadLanguage;
        }
...
// requirejs json plugin
    ...
    if (language && map[language]) {
        req.push('/customize/translations/messages.' + language + '.js');
    }
                    ...
                    var checkInObject = function (ref, translated, path) {
                        ...
                        Object.keys(ref).forEach(function (k) {
                            if (/^updated_[0-9]+_/.test(k) && !translated[k]) {
                                ...
                                if (updated[key]) {
                                    var ek = updated[key];
                                    if (parseInt(ek.split('_')[1]) > parseInt(k.split('_')[1])) {
                                        return;
                                    }
                                }
                                updated[key] = k;
